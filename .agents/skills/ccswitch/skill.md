---
name: ccswitch
description: "cc-switch Provider 切换工具。在多个 AI 模型 Provider 之间切换，并自动处理 Codex Pro 订阅修复。示例：\"换成 MiniMax\", \"切到 DeepSeek\", \"用官方订阅\", \"切回 Codex\", \"看看有哪些模型\""
---

# cc-switch Provider 切换工具

在 Codex / Codex / OpenClaw 的多个 AI Provider 之间切换，自动处理订阅缓存。
...
| 切回官方订阅 | 切回 Codex Official + 自动修复订阅缓存 |
...
| App | Provider | 说明 |
|-----|----------|------|
| Codex | **Codex Official** | 原生 Pro 订阅 |
| Codex | OfoxAI | `api.ofox.ai` 中转 |
| Codex | DeepSeek | DeepSeek V3.2 |
...
2. 执行对应命令
3. **如果切回 Codex Official，必须执行 fix-sub**
4. 提示用户重启 Codex（如果切回官方订阅）
...
"$PYTHON" "$SCRIPT" use DeepSeek Codex
"$PYTHON" "$SCRIPT" use OfoxAI

# 切回 Codex 官方订阅（自动修复缓存）
...
### 切回 Codex Official（重要！）
**必须用 `fix-sub`，不能用 `use "Codex Official"`。**

原因：切第三方时 Codex 会缓存 `hasAvailableSubscription: False`，
`fix-sub` 会同时：
1. 切换到 Codex Official
2. 清除 `~/.Codex/settings.json` 中的第三方 env 注入
3. 重置 `~/.Codex.json` 的订阅缓存

执行完 `fix-sub` 后，**提示用户重启 Codex**。
...
1. 切回 Codex Official 后必须**重启 Codex**才能生效
