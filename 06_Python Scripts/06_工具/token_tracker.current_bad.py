...
用法:
  python token_tracker.py                    # 紧凑 ASCII 仪表盘（默认）
  python token_tracker.py --rich             # Rich 终端字符画仪表盘（旧格式）
...
def load_official_billing(amount_path, cost_path, model='deepseek-v4-pro'):
    ...
    with open(amount_path, 'r', encoding='utf-8') as f:
        for row in csv.DictReader(f):
            ...
            if typ == 'request_count':
                continue
            tokens_by_date[row['utc_date']][row['model']][typ] += int(row['amount'])
    ...
    for d in sorted(all_dates):
        if model in cost_by_date[d] or model in tokens_by_date[d]:
            result[d] = {
                'cost': cost_by_date[d].get(model, 0.0),
                'tokens': dict(tokens_by_date[d].get(model, {})),
            }
    return result
...
def is_csv_imported(conn, amount_hash, cost_hash):
    cursor = conn.cursor()
    cursor.execute('''
        SELECT id FROM official_billing_imports
        WHERE amount_hash = ? AND cost_hash = ?
    ''', (amount_hash, cost_hash))
    return cursor.fetchone() is not None


def mark_csv_imported(conn, amount_hash, cost_hash, amount_path, cost_path):
    cursor = conn.cursor()
    cursor.execute('''
        INSERT OR IGNORE INTO official_billing_imports (amount_hash, cost_hash, amount_file, cost_file)
        VALUES (?, ?, ?, ?)
    ''', (amount_hash, cost_hash, str(amount_path), str(cost_path)))
    conn.commit()
...
def update_ema_calibration(amount_path, cost_path, raw_estimated_month_cost, billing_data):
    ...
    if is_csv_imported(conn, a_hash, c_hash):
        conn.close()
        return None
    ...
    mark_csv_imported(conn, a_hash, c_hash, amount_path, cost_path)
    conn.close()
...
def resolve_billing_context(raw_est_month_cost, cache):
    ...
    if amount_path and cost_path:
        billing_data = load_official_billing(amount_path, cost_path)
        if billing_data:
